	...
	phi(_adist(_rng))
{
	...
	if (!IsStaticInitialized())
	{
		...
		AddStaticBind(std::make_unique<VertexBuffer>(_gfx, vertices));
		...
		AddStaticIndexBuffer(std::make_unique<IndexBuffer>(_gfx, indices));
		...
		AddStaticBind(std::make_unique<PixelConstantBuffer<ConstantBuffer2>>(_gfx, cb2));
